...
async function createProduct(req, res) {
  try {
    const { product_name, description, price, stock, image, category_id } =
      req.body;
    ...
    if (
      ...
      return res
        ...
        .json({
          message:
            "Please provide product_name, description, price, stock, image and category_id",
        });
    }
    ...
    if (
      ...
      return res
        .status(400)
        .json({
          message: "product_name, description, and image must be strings",
        });
    }
    ...
    if (typeof category_id !== "number" || isNaN(category_id)) {
      return res
        .status(400)
        .json({ message: "Category_id must be a valid number" });
    }
...
async function updateProduct(req, res) {
  try {
    const { product_id } = req.params;
    const { product_name, description, price, stock, image, category } =
      req.body;
